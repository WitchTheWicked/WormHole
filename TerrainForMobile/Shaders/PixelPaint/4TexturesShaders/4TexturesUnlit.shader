Shader "TerrainForMobile/4TexturesUnlit" {
Properties {
...
SubShader {
    Pass {
		...
		v2f vert (appdata_base v)
		{
			...
			o.uv[3] = TRANSFORM_TEX (v.texcoord, _Layer4);
		...
		{
			half4 Mask = tex2D( _MainTex, i.uv[4].xy );
			half4 lay1 = tex2D( _Layer1, i.uv[0].xy );
			half4 lay2 = tex2D( _Layer2, i.uv[1].xy );
			half4 lay3 = tex2D( _Layer3, i.uv[2].xy );
			half4 lay4 = tex2D( _Layer4, i.uv[3].xy );
   				
    		half4 c;
			c.xyz = (lay1.xyz * Mask.r + lay2.xyz * Mask.g + lay3.xyz * Mask.b + lay4.xyz * Mask.a) * _Color.rgb * 2;
			c.w = 0;
			return c;
		}
		ENDCG
